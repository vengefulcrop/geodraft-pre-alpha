"""Toolbar WorkSpaceTool entry for "Draw Capsule"."""
...
OVERLAY = PlacementOverlay(
    ...
    veil_inward=True,
    hint=(
        "click centre, radius, centre, radius  |  type numbers  |  "
        "F vertex count  |  Ctrl snap  |  Shift no grid  |  V freeze  |  "
        "X/Y/Z plane  |  N surface  |  Alt subtract"
    ),
)


class VIEW3D_T_geodraft_capsule_draw(WorkSpaceTool):
    ...
    bl_description = (
        ...
        "Ctrl to snap to geometry, Shift to ignore the grid"
    )
    ...
    bl_keymap = (
        ...
        (
            MESH_OT_geodraft_capsule_draw.bl_idname,
...
def register():
    OVERLAY.register()
    register_tool(VIEW3D_T_geodraft_capsule_draw, separator=False)


def unregister():
    OVERLAY.unregister()
    drop_stale_cursor(TOOL_IDNAME)
    unregister_tool(VIEW3D_T_geodraft_capsule_draw)
